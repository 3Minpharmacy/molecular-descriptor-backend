...
# PYTHONPATH must include /app so that `from app.routes import ...`
# resolves correctly regardless of how uvicorn is invoked.
ENV PYTHONPATH="/app"

# Copy the full project root (not just the app/ subdirectory) so that
# the app package and any sibling files are all on the Python path.
...
# Single worker is used here for compatibility with Render's starter plan.
# Scale horizontally via Render's instance count setting, not --workers.
